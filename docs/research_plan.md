...
| `e81_pressure_comovement.py` | C2 | **pre-registered**: does `projection_pressure` CO-MOVE with the control's excess, within a seed? | **launched, prediction before the run.** `e80`'s +0.767 is a rank correlation **across nine partitions** — it says partitions with a bigger pressure spread tend to have a bigger excess spread, not that the two quantities move together on the *same relabelling*. That is the difference between a lead and a mechanism, and `e75` already showed how to test it (centre by the seed mean, correlate within a seed so the task draw is held fixed). **Prediction:** mean within-seed co-movement **≥ +0.5**, mean *r*² **≥ 0.25**, and better than the alignment's **+0.170** from `e75`. **Falsifier:** a mean co-movement **at or below +0.170**, i.e. no better than the bare alignment's — which would demote `e80`'s +0.767 to a coincidence of nine rank positions rather than a relationship between the two quantities. It also recomputes the spread statistic on the same data, so `e80`'s headline is reproduced inside one artifact rather than across two. ~35 min. The probe's two partitions read +0.940 / +0.984 — **but at three draws a within-seed correlation has one degree of freedom, so the probe's values are degenerate and only the full six-draw run is a measurement** |
