...
| `e75_task_pair_spread.py` | C2 | **pre-registered**: does the control's excess MOVE WITH the partition's alignment? | **launched, design fixed before the run.** `e67` refuted the one-scalar model and `e72` refuted its replacement, and all three failed candidates (group count, concentration, alignment spread) are **functions of the partition alone** while the draw spread is a *(partition, task)* property. `e72`'s diagnosis says how to differ in two ways, and this does both: **no chance denominator** (the bare `alignment_score`, since the generic-subspace normalisation is what stripped the task information) and **co-movement rather than spread** (a quantity can have a large spread and no relationship to the excess — which is what `e72` found). **And the task draw is held fixed**: both variables move a great deal with the task geometry, so every correlation is computed *within a seed*, across draws, after centring each variable by its own seed mean — a pooled `(draw, seed)` correlation would be dominated by the seed dimension and would answer a question about the tasks. `e72` already measured this candidate's *spread* form (`raw_alignment_spread` = −0.500, wrong in sign), so the co-movement is the genuinely new quantity. **Prediction:** within-partition co-movement ≥ +0.5, and its ordering across partitions tracks the measured draw spread at Spearman ≥ +0.8, with a concentration correlation below +0.5. **Falsifier:** a co-movement at or below zero, which would say the control's excess is insensitive to *which* neurons share a group even though the alignment is not. The probe's first two partitions read +0.943 (`cell_type` min 1) and +0.314 (min 2) |
| `e3_basis_selection --circuit-size 1500 --ladder` | C2 | per-seed storage for the **d = 1874** granularity ladder (`e79`), the second configuration of the headline family | **launched** — `runs/e79_ladder_d1874_perseed.json`, byte-for-byte the configuration of `runs/e9_ladder_d1874.json` (cs 1500, support 150, 12 seeds, ladder, no-realized) which stores **no** `excess_per_seed` on any of its 18 bases. 5.6 h. It closes the same rule-8 gap `e58` closed at d = 1307, at the second configuration — where `e9`'s finding found the ladder replicates **in form and not in number**, which is exactly the claim a per-seed check should test |
