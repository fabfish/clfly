...
| `e8_rate_network` at the other two settings, pool 96 / per-step 8 | C2b | the task-IL and class-IL replay arms (`e69`) — the two pool-96 numbers `e61` does not cover | **queued behind the CPU queue** — task-IL is the default (per-task heads, `--readout-size 0`) and class-IL is `--shared-head --readout-size 0`. Neither has an artifact: a census of the 17 stored artifacts carrying a `naive` arm finds `replay_per_task: 96` in **exactly one**, `e61`'s own step-8 run. The task-IL recreation is *validatable*, because the settled finding's task-IL naive (+0.101) matches `e10_rung_*`'s naive **0.8241 / +0.1007** exactly; class-IL's claimed naive (+0.059 ± 0.028) matches **no** stored artifact, so that row can be re-measured but not confirmed against a fingerprint |
...
| `e75_task_pair_spread.py` | C2 | **pre-registered**: a draw-spread predictor that is a function of the *(partition, task)* pair rather than of the partition alone | **queued, prediction before the run.** Three candidates have now failed — group count, concentration and the alignment spread — and **all three are functions of the partition alone**, while the draw spread is how much the *excess* moves under relabelling and the excess is a filter quantity computed against the tasks. The candidate is the spread across draws of the **absolute** overlap between the indicator span and the task's own top-`r` spectral subspace, `||Q^T U_trunc||`, with **no chance denominator** — the normalisation is what threw the task information away in `e72`. **Prediction:** Spearman ≥ +0.8 with the measured draw spread *and* below +0.5 with concentration. **Falsifier:** a concentration correlation as high as the alignment spread's +0.85 |
| `e12_control_spread.py --column cell_type --min-size {8,16,128}` | C2 | the three ladder rungs whose draw sd has never been measured (`e74`) — the floor under `e73`'s head-to-head | **launched** — `runs/e74_drawsd_min{8,16,128}.json`, at `--seeds 3 --draws 5` to match `e14`'s protocol exactly for the points already measured. `pool32`/`pool64` are covered by the measured point at concentration 0.325; these three are the rest |
...
| `e77_thread_determinism_probe.py` | — | is the rate-network benchmark reproducible across runs and across thread counts? | done — **`repeats` is inert, the thread count is not.** Same command, `repeats 3` vs `4`: the first three replicates agree exactly. Same command, `OMP_NUM_THREADS=4` vs default: **0 of 3 accuracy values shared**. So the benchmark is deterministic *given an environment* and not across environments. Consequence: `e54`'s determinism control is scoped to a thread setting, which no artifact records, and the project's own runs are split across settings — every cross-artifact comparison inherits a variable nothing stored. The analytic (numpy) path looks immune on one observation: `e65`'s realization 0 reproduced `e48`'s value to six decimals under a different setting |
