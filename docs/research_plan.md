...
| `e81_pressure_comovement.py --circuit-size {300,1500}` | C2 | **pre-registered**: does the mechanism replicate at two other circuit sizes? (`e82`) | **half done: d = 952 replicates to ±0.02.** Mean *r* **+0.923** against +0.906 at d = 1307, mean *r*² **0.854** against 0.828, non-positive co-movements **0 of 9 at both sizes**, and **all 54 seed-level correlations across the two runs positive, smallest +0.636** — the paired size difference is **+0.017 ± 0.020, sign p = 1.000**. And the one weak partition reverses: `side`, the weakest at d = 1307 (*r*² 0.464), is among the stronger at d = 952 (*r*² 0.640), so that caveat belongs to d = 1307 and not to `side`. The replication also exposes the project's own crowding — at cs = 300 `cell_type` min 4 and min 6 are the **same partition**, so there are eight distinct and not nine — which changes nothing (dedup gives +0.916 / 0.841) but is stated. **The d = 1874 half is still running, and the *spread* statistic is not replicated at all**: the script now refuses to print it away from d = 1307, because its target is a foreign number at another circuit size |
