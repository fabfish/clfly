...
| `e8_rate_network.py` | — | the non-linear substrate, both settings, frozen-body control | **re-opened** — its headline positive result (replay, pool 96 / per-step 8, −0.010 ± 0.006 = 4.2σ) has **no artifact**: all 77 runs store `replay_per_task: 16` and `replay_batch: 16`. The nearest available instance (pool 16, n = 5) gives **−1.17σ with `+----` signs and a LOO σ range of [0.67, 5.66]**. `e61` recreates the whole budget sweep |
...
| `e8_rate_network --replay-per-task 96 --replay-batch {8,16,48}` | C2b | recreate the missing artifact behind the network line's headline replay result (`e61`) | **in flight** — `runs/e61_replay96_step{8,16,48}.json`; the reconstructed config's `naive` arm matches `e8_hardened` exactly (0.914 / +0.073), so the comparison will be like-for-like |
