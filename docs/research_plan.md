...
| `e12_control_spread.py --column {side,cell_type}` | C2 | measure the two draw-sds that the predictor's correction had to *interpolate* (`e67`) | **in flight** — `runs/e67_drawsd_{side,cell_type}_min1.json`; every coarse-rung σ in the study, and `side`'s 15.53σ → 4.47σ correction in §5, currently rests on that interpolation |
