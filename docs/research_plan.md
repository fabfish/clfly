...
| `e81_pressure_comovement.py --circuit-size {300,1500}` | C2 | **pre-registered**: does the mechanism replicate at two other circuit sizes? (`e82`) | **launched, prediction before the run.** Every new result in this project gets a second configuration, and this is the cheapest such test the project has: **the co-movement needs no measured draw sd at all**, because it is a within-partition correlation between two quantities computed on the same relabelling — so unlike `e80` it has nothing to re-measure at a new size. Sizes: **cs = 300 (d = 952)**, which is `e6`'s `baseline` condition, and **cs = 1500 (d = 1874)**, which is `e9`/`e79`'s ladder size. **Prediction:** the co-movement replicates at both — mean *r* **≥ +0.5** at each, and the alignment comparison run at the same size is beaten at each. **Falsifier:** a mean *r* **below +0.5** at either size, which would make `e81`'s 0.828 a property of the d = 1307 task suite rather than of the quantities. Guards added to the script for the replication: it **refuses to report the spread statistic** away from d = 1307, since that target is a foreign number, and it skips a pooling that collapses the circuit to fewer than two groups |
