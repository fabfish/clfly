...
| `e75_task_pair_spread.py` | C2 | **pre-registered**: a draw-spread predictor that is a function of the *(partition, task)* pair rather than of the partition alone | **its first form is already refuted; re-specified below, not yet run.** Three candidates have failed — group count, concentration and the alignment spread — and **all three are functions of the partition alone**, while the draw spread is how much the *excess* moves under relabelling and the excess is a filter quantity computed against the tasks. The first candidate written here was the spread across draws of the **absolute** overlap `||Q^T U_trunc||` with no chance denominator — and **`e72` already measured exactly that**: its `raw_alignment_spread` scores **−0.500 (p = 0.17)**, i.e. wrong in *sign*, so removing the denominator is not the fix. **Re-specified:** the genuinely new quantity is the *co-movement* rather than the spread — for each partition, do the raw alignment and the control's excess move together across the `D × S` (draw, task-seed) pairs? Nothing on disk can answer it, because `e12` stores only per-draw *means* of the control (`control_means`), never per-(draw, seed) values. **Prediction:** the within-partition correlation is positive (≥ +0.5) and its ordering across partitions tracks the measured draw spread at Spearman ≥ +0.8. **Falsifier:** a within-partition correlation at or below zero, which would say the control's excess is insensitive to *which* neurons share a group even though the alignment is not |
