...
| `e8_rate_network --replay-per-task 96 --replay-batch {8,16,48}` | C2b | recreate the missing artifact behind the network line's headline replay result (`e61`) | **two of three done, and both reproduce.** Per-step 8: −0.08542 ± 0.01293 = **6.61σ paired**, 5/5 negative. Per-step 16: −0.06250 ± 0.01545 = **4.05σ**, 5/5 negative. The **budget inversion** — the claim's other half — holds where checkable: `16 − 8` = **+0.02292 ± 0.00390 = 5.88σ paired**, 5/5 positive, LOO σ [4.70, 8.66], no flips, leverage 0.80. Per-step 48 still running |
| `e61_replay_recreation.py` | C2b | the recreated sweep against the published claim, with paired per-replicate structure | done for per-steps 8 and 16 — `runs/e61_replay_recreation_analysis.json`; §3 reports the inversion as a **paired** contrast rather than as two means, because the per-step arms share their replicate seeds. Re-runnable as the last draw lands, no code change needed |
