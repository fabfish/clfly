...
what *creates* the ladder rather than what moves its step — so the mechanism was open on a variable nobody had then
varied (`docs/findings/2026-09-26-the-circuit-size-sets-the-one-side-level.md`). **And the one scalar every cell of this table shares had never been moved**: the task builder's assembly geometry (`stable_weights`' target spectral radius, `0.9`), which can be varied at a *fixed* circuit size — the one comparison the size dependence has never had. A one-seed smoke test at cs `300` gives `real` 0.031800 and Erdős–Rényi 0.154173 at `rho` `0.9` against `real` 0.000315 and Erdős–Rényi 0.023982 at `rho` `0.5`: the floor collapses 100× while the top rung falls 6.4×, so the ratio moves 16× from one scalar at one size, and the size dependence is therefore being tested against that scalar rather than against another size (`docs/findings/2026-09-26-registered-the-assembly-geometry-against-the-top-step.md`). **And that variable moves the top step further than the circuit size does**: at cs 300, holding the circuit fixed, the top step is **1.018×** at `rho` 0.5, **1.280×** at 0.9 and **11.774×** at 0.99 -- a **11.56×** span at one size against the **2.2×** between cs 300 and cs 800 -- so the size dependence above is a 2.2× difference measured inside an 11.6×-wide dependence on a parameter this line had never moved; and the registered direction (a fall with `rho`) was refuted, its falsifier firing, because the one-side level collapses faster than the top rung at `rho` 0.99 (0.12240 to 0.00686) and the ladder's order inverts at `rho` 0.5 (`docs/findings/2026-09-26-the-assembly-geometry-moves-the-top-step.md`). **And what carries that dependence is not the alignment contrast**: across the cells that vary `rho` the penalty top step spans **11.56×** while the alignment top step -- Erdős–Rényi's alignment against chance over the one-side nulls' -- spans **1.13×**, so the two-side-versus-one-side alignment contrast is flat at 1.15–1.30× in every cell; what orders like the penalty is the task geometry's **shape**, the one-side nulls' effective rank running **26.17 → 9.41 → 1.07** over `rho` 0.5/0.9/0.99 (a rank contrast of 1.02× → 1.73× → 3.25× against the penalty's 1.02× → 1.28× → 11.77×) -- monotone together but **not** carrying the 11.5× magnitude, so the shape is where to look next and the alignment is where not to, and at `rho` 0.99 a one-side null's task geometry is essentially one-dimensional (`flattening` 0.036 against 0.884) (`docs/findings/2026-09-26-the-top-steps-rho-dependence-is-not-the-alignment-contrast.md`). **And the completed grid says the size dependence is not one effect**: cs 800 over cs 300 reads 1.13× at `rho` 0.5, 2.14× at 0.9 and **0.06×** at 0.99 — the *sign* of the size effect is set by `rho` — and the cs-800 cells at `rho` ≠ 0.9 read **1.151×** and **0.748×** against the table's 2.737×, i.e. 0.27×–0.42× of it, so the 7× headline is a `rho = 0.9` statement; at cs 800 and `rho` 0.99 the two one-side families come apart by **8.7×** (`alloy1` 0.11395 against `inalloy1` 0.01312, ranks 21.01 against 1.29), so at high `rho` the rank collapse is **directional** and the one-side level is a mean over quantities that have separated -- with the qualification `e230` measured: at cs 300 all four families' rank contrasts resolve their own drawing scatter (7.68× to 25.25× against 1.06× to 2.73×) -- **WITHDRAWN 2026-09-26 18:20**: `e253`'s second drawing at rho 0.95 and 0.98 collapsed those cs-300 contrasts to 1.16× to 4.05× against the same 1.06× to 8.77× scatters, so five of the six cs-300 families are now unresolvable and `e230` declares them; the cs-800 half is unchanged and at cs 800 the collapsing family's does (44.79× against 4.14× (45.14× against 8.02× once `e243`'s three designed drawings enter the census, so the margin is 5.6× where it was 10.8×) while the `alloy1` side of that comparison does **not** (twelve drawings spanning 16.44× against a 3.01× single-drawing contrast), so volatility belongs to the family and not to the statistic. **And the curve itself is now measured at cs 300 on a nine-point `rho` grid**: every family falls monotonically — `real` 27.25 to 1.48, `alloy1` 26.52 to 1.00, `inalloy1` 25.82 to 1.00, `erdos_renyi` 26.75 to 3.18 over `rho` 0.5 to 0.999 — the two one-side families **cross** between `rho` 0.7 and 0.8 (so below about 0.75 the sides are interchangeable and the asymmetry's direction is a property of the high-`rho` regime), and `erdos_renyi` **does not collapse**: it plateaus at 3.18 while the others reach 1.0, so what survives near-critical propagation is decided by whether the destruction is edge-wise random on *both* sides rather than by how much degree structure was destroyed. **And the penalty does not follow that geometry**: over six `rho` values at cs 300 the rank falls monotonically while the excess **rises to a peak at `rho` ≈ 0.9** and then collapses — so the two curves' shapes disagree from the first step — and the two-side advantage is **late** rather than high-`rho` (Erdős–Rényi over `alloy1` is 1.91× at `rho` 0.95, 5.53× at 0.98, 14.77× at 0.99), switching on *above* the point where the rank falls steepest. What is monotone is the **ratio**: the top step runs 1.06× to 11.77× across the grid because the one-side level collapses 18× past its peak while Erdős–Rényi only halves. **And the asymmetry's sign turns out to be a DRAWING's, not the substrate's**: across three drawings of the null the cs-300 `alloy1`/`inalloy1` ratio at `rho` 0.9 reads 0.62×, **1.78×** and **0.31×** (mean 1.05×, both signs, spanning a factor of 2.8) and at cs 800 it reads 10.10×, 1.06× and 0.23× — so the separation is systematic *within* a drawing and not across them, and the other half of the grid stands:  `erdos_renyi`'s plateau is the *size-invariant* quantity, 3.18 against 3.73 at `rho` 0.999 where `real` goes 1.48 against 10.03 (`docs/findings/2026-09-26-the-rho-sweep-is-complete-and-the-size-effect-changes-sign.md`). **And the two-side level is
 ...
 **And the ladder's axis orders the LEVEL and not the spread**: pooling every (cell, family) group that carries two or more drawings, the median excess spread orders exactly as the ladder predicts — 1.45× for the nulls that destroy no degree sequence, 1.29× for the one-side families and 1.03× for Erdős–Rényi, with the kind-2 and kind-0 ranges disjoint — but inside the one cell that carries all three levels **the order reverses**: the one-side families' median 2.96× against the no-destruction families' 1.45×, because the no-destruction rung is measured at two cells while the one-side median sits on fourteen groups over six cells — the second cell (`e245`, cs 300/support 30) gives the same sign **47×** smaller, kind 1 1.42× against kind 0's 1.39× (+2.2%, inside the registered null band), so kind 0's spread is the stable one (1.39× and 1.45×, a span of 1.04×) while kind 1's swings 2.74×; and the two-side kind's tightness cannot be separated from its height, since it is both the tightest (6 of 6 cells) and the highest, while on the rank spread that same cell orders the three levels the other way (5.87×, 3.26× and 1.23×), so the penalty's spread and the geometry's disagree about which construction is loosest (`docs/findings/2026-09-26-the-spread-does-not-follow-the-destruction-count.md` (both figures re-read on `e243`'s three designed drawings, which landed after this was written: the one-side median moves 1.234× to 1.292× and the reversal 2.29× to 2.96×, so the reversal widens rather than dissolving)).
...
1. **Replicate the granularity curve elsewhere, and finish it on synapses.** The neuron
   ...
   re-measured, and the plateau moved by a factor of two**: in the **matched-control quantity** the same eight rungs
   at twelve seeds each put the minimum at **`pool4`** on d = 1307 (−0.008844 ± 0.000199) and at **`pool8`** on
   d = 1874 (−0.007763 ± 0.000169), each **one rung wide and interior**, with `pool1` (no pooling) the worst rung on
   both. **The mechanism the shift is consistent with is a scale one**: `pool_below` is an **absolute neuron count**,
   so the same rung merges fewer labels on a larger circuit — and the circuits differ by **1.43×** while the
   optimum moved by **2×**, which is the direction and roughly the size that predicts. **A first version of this
   paragraph said sixteen times and "no interior optimum on the second circuit", and both were artefacts of
