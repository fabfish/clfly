...
def test_the_live_corpus_halves_the_quoted_spreads_and_shares_the_noise():
    ...
    `e248`'s, `e249`'s and `e251`'s new drawings move from +0.724 to +0.711."""
    p = Path("runs/e247_count_matched_spread.json")
    if not p.exists():
        return
    d = json.loads(p.read_text(encoding="utf-8"))
    rows = {r["id"]: r for r in d["claims"]}
    # the rho-0.99 cell (e251) moves R1 and R2 to their falsifiers: its one-side scatter is 9x to 32x, so
    ...
    assert "15.33x" in rows["R1"]["measured"] and "3.47x" in rows["R1"]["measured"], rows["R1"]
    assert "1.879x" in rows["R2"]["measured"], rows["R2"]
    assert "overlapping=True" in rows["R2"]["measured"], rows["R2"]
    assert "+0.711" in rows["R5"]["measured"], rows["R5"]
